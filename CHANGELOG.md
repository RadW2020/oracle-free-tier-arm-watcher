...
### 📨 Despliegue Postiz "Ultimate"
- **Despliegue completo** de Postiz App en Coolify sobre OCI (ARM64).
- **Arquitectura Distribuida:** Postiz + Temporal + Elasticsearch + Postgres + Redis.
- **Configuración Manual:** Inyección de Docker Compose personalizado para evitar sobrescrituras de Coolify.
- **Configuración Temporal:** Inyección de `development-sql.yaml` en el host para gestión de cuotas.
- **Optimización de Puertos:** Reasignación de Temporal UI al puerto 8085 para evitar colisiones.
- **Documentación:** Creados `POSTIZ_DEPLOYMENT.md` y `POSTIZ_COMPOSE_REFERENCE.yaml`.
- **Almacenamiento:** Configuración base para OCI Object Storage compatible con S3.
